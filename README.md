...
**Option A (recommended):** run the demo script in `scripts/`

> If your demo script name differs, run `ls scripts` and use the dispatcher/demo entrypoint you created.
...
python3 -u scripts/dispatcher_demo.py urls.txt
```

**Option B:** run a module entrypoint (if you built one)

```bash
PYTHONPATH=src python3 -m ratelimmq.dispatcher_demo urls.txt
```
